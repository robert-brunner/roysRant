...
export const DEFAULT_SCROLL_INTO_VIEW_OPTIONS = {
  block: "nearest",
  inline: "nearest",
};
 ...
 */
export function isFindAndReplaceAvailable(editor) {
 ...
 */
export function getFindAndReplaceStorage(editor) {
 ...
 */
export function formatResultCount(currentIndex, total) {
 ...
 */
export function scrollCurrentResultIntoView(editor, scrollIntoViewOptions = DEFAULT_SCROLL_INTO_VIEW_OPTIONS) {
 ...
 * ```
 */
export function useSearchAndReplace(config) {
  const {
  ...
  const setReplaceTerm = useCallback((value) => {
  ...
  const toggleCaseSensitive = useCallback(() => {
    if (!editor || !isFindAndReplaceAvailable(editor)) return
    editor.commands.setCaseSensitive(!getFindAndReplaceStorage(editor)?.caseSensitive)
  }, [editor])
